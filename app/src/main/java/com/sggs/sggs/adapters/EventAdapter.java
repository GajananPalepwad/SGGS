...
public class EventAdapter extends RecyclerView.Adapter<EventAdapter.ViewHolder> {
    ...
    private  ArrayList<EventModel> timeList;
    private  Context context;

    private List<Integer> colorList;
    public EventAdapter(ArrayList<EventModel> timeList, Context context) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        EventModel event = timeList.get(position);

        String[] parts = event.getTitleNbody().split("\\(");
        String titleString = parts[0].trim();
        String bodyString = parts[1].split("\\)")[0].trim();
        holder.time.setText(event.getTime().replace(", ", "\n"));
